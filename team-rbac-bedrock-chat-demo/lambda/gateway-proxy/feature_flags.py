...
def _get_client():
    """Lazy-initialize the AppConfig client (avoids NoRegionError at import time)."""
    global _client
    if _client is None:
...
def get_flags():
    ...
    try:
        if not _session_token:
            session = _get_client().start_configuration_session(
        ...
        response = _get_client().get_latest_configuration(ConfigurationToken=_session_token)
